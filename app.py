...
# IMPORTANTE: Reemplaza con la URL de tu pestaña 'mantenimiento' (la que tiene el gid=...)
URL_DIRECTA_MANTO = "TU_URL_AQUI_CON_EL_GID" 

# 2. Función de limpieza de datos (Pestaña Vertimiento - LÓGICA BASE)
def limpiar_datos_ptar(df):
    if df is None or df.empty:
        return pd.DataFrame()
    
    df.columns = df.columns.str.strip()
    # Mapeo flexible para asegurar que las métricas funcionen
    mapeo = {
        'ph': 'ph', 'pH': 'ph', 'PH': 'ph',
        ...
        if not df_manto.empty:
            ...
            if 'EQUIPO' in df_manto.columns:
                ...
                for i, eq_nombre in enumerate(equipos):
                    ...
                    # Manejo de nombres de columna flexibles (Marca temporal o Fecha)
