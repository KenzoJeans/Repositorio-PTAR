...
# --- CONFIGURACIÓN Y ESTILO DE BARRA LATERAL ---
    ...
    # MODIFICACIÓN PRO: Inicializamos la variable rango en None para evitar NameErrors globales
    rango = None
    ...
    if isinstance(rango, (list, tuple)) and len(rango) == 2:
        ...
        # ← AÑADIR: Filtro para Mantenimiento
        if not df_manto.empty:
            # Primero normalizar columnas y parsear fechas
           ...
           # FILA 3: Análisis de Temperatura
            ...
            with col3:
                ...
                fig_temp_hist.update_traces(
                    ...
                    fillpattern_fgcolor="rgba(255, 87, 34, 0.8)", 
    ...
    with t2:
        ...
        if not df_tratada.empty:
            ...
            with col_c:
                ...
                fig_rem = px.bar(df_comp, x='Etapa', y='SST (mg/L)', color='Etapa', color_discrete_map={'Entrada (Crudo)': '#78909C', 'Salida (Tratada)': '#00E676'}, template="plotly_dark")
            ...
            st.warning("No hay datos registrados en la hoja de 'Agua Tratada' para este rango.")
    ...
    with t3:
        ...
        if not df_manto.empty:
            ...
            # MODIFICACIÓN PRO: Forzar la conversión de fechas a datetime.date para comparar con Streamlit
            df_manto[col_fecha_m] = pd.to_datetime(df_manto[col_fecha_m], errors='coerce').dt.date
            
            # MODIFICACIÓN PRO: Aplicar el filtro de rango de fechas global a la hoja de mantenimiento
            if rango is not None and isinstance(rango, (list, tuple)) and len(rango) == 2:
                inicio, fin = rango
                df_manto = df_manto[(df_manto[col_fecha_m] >= inicio) & (df_manto[col_fecha_m] <= fin)]
            
            # Control por si el rango de fechas deja vacío este dataset específico
            if not df_manto.empty:
                ...
                if 'EQUIPO' in df_manto.columns:
                    ...
                    for i, eq in enumerate(equipos):
                        df_eq = df_manto[df_manto['EQUIPO'] == eq]
                        if not df_eq.empty:
                            # Captura el último registro válido DENTRO de las fechas elegidas
                            ult_reg = df_eq.iloc[-1]
                            val_s = ult_reg['SALUD']
                ...
                with col_v2:
                    ...
                    if not pendientes.empty:
                        ...
                        st.success("✅ Todos los equipos operan en rangos seguros para este periodo.")

                # --- 4. TABLA DE BITÁCORA ---
                with st.expander("📝 Ver historial filtrado de intervenciones"):
                    st.dataframe(df_manto.sort_values(col_fecha_m, ascending=False), use_container_width=True)
            else:
                st.warning("No hay registros de mantenimiento ejecutados en el rango de fechas seleccionado.")
        else:
            st.warning("No hay datos registrados en la hoja de Mantenimiento.")
    ...
    with t4:
        ...
        if not df_kardex.empty:
            ...
            df_k_periodo['CANTIDAD'] = df_k_periodo['CANTIDAD'].astype(str).str.replace(',', '.').str.replace(r'[^0-9.]', '', regex=True)
            ...
            # MODIFICACIÓN PRO: Cambiado df_kardex por df_k_periodo para sincronizar la dona con las fechas
            df_salidas = df_k_periodo[df_k_periodo['QUE PROCESO VA A REALIZAR'] == 'SALIDA'] if 'df_k_periodo' in locals() else df_kardex[df_kardex['QUE PROCESO VA A REALIZAR'] == 'SALIDA']
            consumo_total = df_salidas.groupby('NOMBRE DEL QUIMICO')['CANTIDAD'].sum().reset_index()

            with col_dona:
                st.write("**🍩 Distribución de Consumo (Periodo)**")
            ...
            st.markdown("---")
            st.write("**📋 Historial Detallado de Movimientos (Periodo)**")
            
            # MODIFICACIÓN PRO: Cambiado df_kardex por df_k_periodo para sincronizar la tabla con las fechas
            df_view = df_k_periodo[['FECHA', 'NOMBRE DEL QUIMICO', 'QUE PROCESO VA A REALIZAR', 'CANTIDAD']].copy()
