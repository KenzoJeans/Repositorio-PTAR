...
try:
    ...
    with t1:
        if not df_vert_filtrado.empty:
            ...
            with col1:
                ...
                fig_ph.add_hline(y=6, line_dash="dash", line_color="#F44336", annotation_text="Límite inf (6)")
                fig_ph.add_hline(y=9, line_dash="dash", line_color="#F44336", annotation_text="Límite sup (9)")
                if len(df_ph) > 2:
            ...
            with col3:
                ...
                fig_temp.add_hline(y=40, line_dash="dash", line_color="#F44336", annotation_text="Máx permisible (40°C)")
                if len(df_tmp) > 2:
                ...
                fig_temp.update_layout(template="plotly_dark", xaxis_title="Fecha", yaxis_title="°C", **LAYOUT_BASE)
            ...
            with col4:
                ...
                fig_tp.add_vline(x=40, line_dash="dash", line_color="#F44336", annotation_text="Límite (40°C)")
    ...
    with t2:
        ...
        if not df_tratada_filtrada.empty:
            ...
            sst_ent     = df_base_full['sst'].replace(0, np.nan).mean() if not df_base_full.empty else None
            ...
            c1.metric("💧 SST Salida (Prom)", f"{avg_sst_sal:.1f} mg/L" if avg_sst_sal else "—", delta=f"⬇️ {remocion:.1f}% Eficiencia")
            c2.metric("⚗️ pH Salida", f"{avg_ph_t:.2f}" if avg_ph_t else "—", delta="✅ OK" if ph_ok_t else "⚠️ FUERA", delta_color="normal" if ph_ok_t else "inverse")
            c3.metric("🌡️ Temp. Salida", f"{avg_temp_t:.1f} °C" if avg_temp_t else "—", delta="✅ ESTABLE" if temp_ok_t else "⚠️ ALTA", delta_color="normal" if temp_ok_t else "inverse")
            c4.metric("📦 Volumen Tratado", f"{total_caudal:.1f} m³" if total_caudal else "—")
            ...
            with col_a:
                ...
                fig_ph_t = px.line(df_ph_t, x='fecha', y='ph', markers=True, template="plotly_dark", color_discrete_sequence=['#00C853'])
                fig_ph_t.add_hrect(y0=6, y1=9, fillcolor="rgba(0,200,83,0.08)", line_width=0, annotation_text="Rango legal (6–9)", annotation_position="top left")
                if len(df_ph_t) > 2:
                    x_n = np.arange(len(df_ph_t))
                    z   = np.polyfit(x_n, df_ph_t['ph'].fillna(0), 1)
                    fig_ph_t.add_scatter(x=df_ph_t['fecha'], y=np.poly1d(z)(x_n), mode='lines', name='Tendencia', line=dict(color='#FFB300', dash='dot', width=1.5))
            ...
            with col_b:
                ...
                fig_temp_t.add_trace(go.Scatter(x=df_temp_t_sorted['fecha'], y=df_temp_t_sorted['temp'], mode='lines+markers', fill='tozeroy', fillcolor='rgba(255,167,38,0.25)', line=dict(color='#FFA726', width=2), name='Temperatura'))
                fig_temp_t.update_layout(template="plotly_dark", xaxis_title="Fecha", yaxis_title="°C", **LAYOUT_BASE)
    ...
    with t3:
        ...
        if not df_manto_filtrado.empty:
            ...
            col_fecha_m2 = next((c for c in df_m.columns if c.strip().upper() == 'FECHA'), df_m.columns[0])
            if df_m[col_fecha_m2].dtype == object:
                df_m[col_fecha_m2] = pd.to_datetime(df_m[col_fecha_m2], dayfirst=True, errors='coerce').dt.date
            ...
            if 'SALUD' in df_m.columns:
                ...
                equi_crit  = (df_m.groupby('EQUIPO')['SALUD'].last() < 6).sum() if 'EQUIPO' in df_m.columns else 0
                equi_prev  = ((df_m.groupby('EQUIPO')['SALUD'].last() >= 6) & (df_m.groupby('EQUIPO')['SALUD'].last() < 8)).sum() if 'EQUIPO' in df_m.columns else 0
                equi_ok    = (df_m.groupby('EQUIPO')['SALUD'].last() >= 8).sum() if 'EQUIPO' in df_m.columns else 0
                    ...
                    with cols_eq[i % 3]:
                        ...
                        <div class="card-equipo" style="border-left: 10px solid {color};">
                            ...
                            <div style="background:#333; border-radius:6px; height:8px; width:100%;">
                                <div style="background:{color}; height:8px; border-radius:6px; width:{barra_w}%;"></div>
                            </div>
                            <p style="font-size:11px; color:#aaa; margin:8px 0 0 0;">Última rev: {fecha_v}</p>
                            <p style="font-size:11px; color:#eee; margin:4px 0 0 0; font-style:italic;">"{observ}"</p>
                        </div>
                        """, unsafe_allow_html=True)
        else:
            st.warning("⚠️ No hay datos de mantenimiento para mostrar.")

    # ══════════════════════════════════════════
    # TAB 4 — QUÍMICOS
    # ══════════════════════════════════════════
    with t4:
        st.subheader("🧪 Control de Consumo de Químicos (Kardex)")
        if not df_kardex.empty:
            st.dataframe(df_kardex, use_container_width=True, hide_index=True)
        else:
            st.info("No hay datos disponibles en el Kardex de Químicos.")

except Exception as e_main:
    st.error(f"❌ Error crítico en la aplicación: {e_main}")
